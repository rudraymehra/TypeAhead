...
/**
 * Frontend logic:
 *  - debounced /suggest calls (avoid a request per keystroke)
 *  - stale-response guard (a slow older request can't overwrite a newer one)
 *  - keyboard navigation (↑ ↓ Enter Esc)
 *  - submit → POST /search → show dummy response → refresh trending/stats
...
$q.addEventListener('input', () => {
  ...
  if (v.length < MIN_PREFIX) {
    latestReq++; // invalidate any in-flight response
    closeDropdown();
    $status.textContent = '';
    return;
  }
...
async function submitSearch(value) {
  ...
  try {
    ...
    // refresh after the batch window so the effect is visible
